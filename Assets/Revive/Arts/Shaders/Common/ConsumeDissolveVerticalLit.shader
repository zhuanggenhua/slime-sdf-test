Shader "Revive/ConsumeDissolveVerticalLit"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        half4 frag(Varyings input) : SV_Target
        {
            ...
            half3 lightColor = mainLight.color * (half)(mainLight.distanceAttenuation * mainLight.shadowAttenuation);
            half3 diffuse = LightingLambert(lightColor, mainLight.direction, normalWS);
            half3 ambient = SampleSH(normalWS);

            half edge = (half)(1.0 - edgeMask);
            half3 lit = baseCol.rgb * (diffuse + ambient);
            lit += edge * _EdgeColor.rgb;

            return half4(lit, baseCol.a);
        }
        ENDHLSL
