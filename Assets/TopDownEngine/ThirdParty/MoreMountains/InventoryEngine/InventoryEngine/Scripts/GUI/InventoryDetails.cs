...
{	
	...
	public class InventoryDetails : MonoBehaviour, MMEventListener<MMInventoryEvent>
	{
		...
		/// the short description container object
		public TMP_Text ShortDescriptionTMP;
		/// the description container object
		...
		protected virtual IEnumerator FillDetailFields(InventoryItem item, float initialDelay)
		{
			...
			#if MM_UGUI2
			if (TitleTMP!= null) { TitleTMP.text = item.ItemName ; }
			if (ShortDescriptionTMP!= null) { ShortDescriptionTMP.text = item.ShortDescription;}
			if (DescriptionTMP != null) { DescriptionTMP.text = item.Description;}
			if (QuantityTMP != null) { QuantityTMP.text = item.Quantity.ToString();}
			#endif
		...
		protected virtual IEnumerator FillDetailFieldsWithDefaults(float initialDelay)
		{
			...
			#if MM_UGUI2
			if (TitleTMP!=null) { TitleTMP.text = DefaultTitle ;}
			if (ShortDescriptionTMP!=null) { ShortDescriptionTMP.text = DefaultShortDescription;}
			if (DescriptionTMP!=null) { DescriptionTMP.text = DefaultDescription;}
			if (QuantityTMP!=null) { QuantityTMP.text = DefaultQuantity;}
			#endif
		}
