...
{	
	...
	public class InventorySlot : Button
	{
		...
		#if MM_UGUI2
		public TMP_Text QuantityTextTMP;
		#endif
		...
		public virtual void SetQuantity(int quantity)
		{
			if (quantity > 1)
			{
				...
				#if MM_UGUI2
				if (QuantityTextTMP != null)
				{
					QuantityTextTMP.gameObject.SetActive(true);
					QuantityTextTMP.text = quantity.ToString();
				}
				#endif
			}
			else
			{
				...
				if (QuantityTextTMP != null)
				{
					QuantityTextTMP.gameObject.SetActive(false);
				}
				#endif
			}
		}
