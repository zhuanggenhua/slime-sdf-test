...
{	
	...
	public class CharacterGridMovement : CharacterAbility 
	{
		...
		/// if this is true, movement will stop when you release input, on the next valid spot
		[Tooltip("if this is true, movement will stop when you release input, on the next valid spot")]
		public bool StopOnInputRelease = true;
		...
		protected virtual void DetermineInputDirection()
		{
			...
			if ((Mathf.Abs(_horizontalMovement) <= IdleThreshold) && (Mathf.Abs(_verticalMovement) <= IdleThreshold) && StopOnInputRelease)
			{
				Stop(_newDirection);
